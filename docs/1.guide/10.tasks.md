...
To run a scheduled task once in the future, you can use `runTask(name, { context: { waitFor: 10 }})` to run the task in 10 seconds.

**Example:**

```ts [server/api/runOnce.ts]
export default eventHandler(async (event) => {
  runTask("future", { payload, context: { waitFor: 10 } });
  
  return { result: "Scheduled to run in 10 seconds" };
});
```

> [!IMPORTANT]
> Scheduled one time tasks are only ever run on full seconds.
> If the actual time would be `YYYY-MM-DD HH:MM:10.250`, the task will be run on `YYYY-MM-DD HH:MM:11`.
