...
Enable [diagnostics channel](https://nodejs.org/api/diagnostics_channel.html) tracing for the server runtime. When enabled, Nitro publishes tracing events from [srvx](https://github.com/h3js/srvx) and [h3](https://github.com/h3js/h3) that can be consumed by APM tools and custom subscribers.

Pass `true` to enable tracing for both `srvx` and `h3`, or an object to toggle them individually.
