...
Enables Node.js [`TracingChannel`](https://nodejs.org/api/diagnostics_channel.html#class-tracingchannel) instrumentation for request handling, publishing `srvx.request` and `h3.request` channels that can be consumed by APM tools and custom subscribers.
