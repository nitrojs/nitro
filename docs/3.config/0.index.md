...
`$development` only applies to the development server. A mount with a different `driver` replaces the whole mount, and one with the same `driver` has its options deep merged.

::note
Previously named `storage`, which still works as a deprecated alias.
::
...
KV storage overrides for development and prerendering. Each mount replaces the matching `kv` mount.
...
export default defineConfig({
  devStorage: {
    redis: {
      driver: "fs",
      base: "./data/redis", // use filesystem in development and prerendering
    },
  },
});
```

:read-more{to="/docs/storage#development-storage"}
