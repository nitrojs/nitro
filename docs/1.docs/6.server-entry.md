...
export default defineHandler((event) => {
  ...
  // Log the request
  console.log(`[${event.context.requestId}] ${event.req.method} ${event.url.pathname}`);
...
- Use the server entry for cross-cutting concerns that affect **all routes**
- Return `undefined` to continue processing; return a response to terminate the request
- Keep the server entry lightweight — it runs for every unmatched request
- Prefer [middleware](/docs/routing#middleware) for modular concerns instead of one large server entry
