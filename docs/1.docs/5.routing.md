...
Route rule matching, normalization, and the `headers` / `redirect` / `proxy` / `cache` / `basicAuth` / `cors` handlers are provided by the [`h3-rules`](https://github.com/h3js/h3-rules) package (extracted from Nitro); Nitro adds the `isr`, `prerender`, and `static` rules on top.
...
> [!NOTE]
> CORS is applied by the running server (h3's `handleCors`), not baked into static/CDN config. On platforms that can serve prerendered/static assets straight from the edge (e.g. Netlify, Vercel), CORS headers are only added when the request reaches the server handler.
