...
> [!TIP]
> You can specify multiple environments or specify a preset name as environment using programmatic registration of routes via [`handlers`](#programmatic-route-handlers) config.
...
In addition to filesystem routing, you can register route handlers programmatically using the `handlers` and `routes` config options.

### `handlers` config

The `handlers` array allows you to register handlers with full control over route, method, middleware, lazy loading, and environment.
...
The `routes` option provides a simpler shorthand for mapping route patterns to handlers:
...
| `handlers` | `NitroEventHandler[]` | `[]` | Programmatic route handlers |
| `routes` | `Record<string, string \| handler>` | `{}` | Shorthand route-to-handler mapping |
