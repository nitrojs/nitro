...
Each rule maps a route pattern (using [rou3](https://github.com/h3js/rou3) syntax) to a set of options. When the `cache` option is set, matching handlers are automatically wrapped with `defineCachedHandler` — see the [cache guide](/docs/cache) to learn more.
