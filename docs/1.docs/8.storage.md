...
::note
`useKV` was previously named `useStorage` and exported from `nitro/storage`, and the `kv` config option was previously named `storage`. The old names still work as deprecated aliases.
::
...
`$development` only applies to the development server. To also override mounts while [prerendering](/docs/prerendering) a production build, use the `devStorage` option instead. Its mounts replace the matching `kv` mounts in development and during prerendering:
