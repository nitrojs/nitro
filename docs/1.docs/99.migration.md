...
::note
`useStorage` has been renamed to `useKV` and `nitro/storage` to `nitro/kv`. The old names still work as deprecated aliases and will be removed in a future version.
::
