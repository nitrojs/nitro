...
2. [Add explicit imports](#auto-imports-are-disabled-by-default) and update [type](#type-imports) and [subpath](#changed-nitro-subpath-imports) imports.
...
::tip
You can opt back into auto-imports by setting the `imports` option in your Nitro config, but explicit imports are recommended for better ecosystem compatibility.
::
