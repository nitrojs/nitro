...
Nitro uses the TypeScript compiler installed in your project to infer JSON-compatible objects, interfaces, arrays, tuples, unions, literals, and optional properties. TypeScript is optional. Without it, request validation and request schema generation still work, while Nitro skips response inference and logs a warning. Response inference only affects the OpenAPI document; Nitro does not validate responses at runtime. Define `openAPI.responses` manually when TypeScript is unavailable or for raw `Response` objects, streams, and types that cannot be represented by JSON Schema. Explicit `parameters`, `requestBody`, and `responses` in `defineRouteMeta` replace the corresponding inferred fields.
