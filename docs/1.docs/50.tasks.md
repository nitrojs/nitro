...
By default, each task can have **one running instance**. Calling a task of the same name multiple times in parallel runs it once, and all callers receive the same return value.
...
For `dedupe` and `serial`, you can provide a `key` function to derive the execution key from the task event. If no key function is provided, Nitro uses the task name.

```ts [tasks/report/generate.ts]
export default defineTask({
  concurrency: {
    mode: "serial",
    key: ({ payload }) => `tenant:${payload.tenant}`,
  },
  async run({ payload }) {
